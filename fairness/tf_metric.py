...
def discrete_demographic_parity(
    index: int, x: tf.Tensor, predicted: tf.Tensor
) -> tf.Tensor:
    """
...
def continuous_demographic_parity(
    index: int, x: tf.Tensor, predicted: tf.Tensor, delta: float = DELTA
) -> tf.Tensor:
    """
    Calculate the demographic parity of a model.
    ...
    probabilities = tf.map_fn(
        lambda value: single_conditional_probability_in_range(
            predicted, protected, value, value + step
        ),
        ...
        lambda value: tf.reduce_min(
            [
                tf.math.divide_no_nan(
                    single_conditional_probability(predicted, protected, value),
                    single_conditional_probability(
                        predicted, protected, value, equal=False
                    ),
                ),
                tf.math.divide_no_nan(
                    single_conditional_probability(
        ...
        lambda value: tf.reduce_min(
            [
                tf.math.divide_no_nan(
                    ...
                    single_conditional_probability_in_range(
                        predicted, protected, value, value + step, inside=False
                    ),
                ),
                tf.math.divide_no_nan(
                    single_conditional_probability_in_range(
...
def discrete_equalized_odds(
    index: int, x: tf.Tensor, y: tf.Tensor, predicted: tf.Tensor
) -> tf.Tensor:
    """
    ...
    masks_a_y_0 = tf.map_fn(
        lambda value: double_conditional_probability(
            predicted, protected, y[:, 0], value, 0
        ),
        unique_protected,
    )
    masks_a_y_1 = tf.map_fn(
        lambda value: double_conditional_probability(
            predicted, protected, y[:, 0], value, 1
        ),
        unique_protected,
    )
    mask_y_0 = tf.math.reduce_mean(tf.boolean_mask(predicted, y_0))
    mask_y_1 = tf.math.reduce_mean(tf.boolean_mask(predicted, y_1))
...
def continuous_equalized_odds(
    index: int, x: tf.Tensor, y: tf.Tensor, predicted: tf.Tensor, delta: float = DELTA
) -> tf.Tensor:
    """
    Calculate the equalized odds of a model.
    The protected attribute is must be binary or categorical.
    ...
    y_0 = tf.equal(y, 0)
    y_1 = tf.equal(y, 1)
    # masks_a_y_0 = tf.map_fn(lambda value: tf.reduce_mean(tf.boolean_mask(predicted, tf.logical_and(tf.logical_and(tf.greater_equal(protected, value), tf.less(protected, value + step)), y_0[:, 0]))), tf.range(min_protected, max_protected, step))
    # masks_a_y_1 = tf.map_fn(lambda value: tf.reduce_mean(tf.boolean_mask(predicted, tf.logical_and(tf.logical_and(tf.greater_equal(protected, value), tf.less(protected, value + step)), y_1[:, 0]))), tf.range(min_protected, max_protected, step))
    masks_a_y_0 = tf.map_fn(
        lambda value: double_conditional_probability_in_range(
            predicted, protected, y[:, 0], value, value + step, 0
        ),
        tf.range(min_protected, max_protected, step),
    )
    masks_a_y_1 = tf.map_fn(
        lambda value: double_conditional_probability_in_range(
            predicted, protected, y[:, 0], value, value + step, 1
        ),
        tf.range(min_protected, max_protected, step),
    )
    # mask_y_0 = tf.math.reduce_mean(tf.boolean_mask(predicted, y_0))
    ...
    mask_y_0 = tf.math.reduce_mean(
        single_conditional_probability_in_range(predicted, y[:, 0], 0, 1)
    )
    mask_y_1 = tf.math.reduce_mean(
        single_conditional_probability_in_range(predicted, y[:, 0], 1, 1)
    )
