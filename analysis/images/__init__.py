...
def plot_fairness_comparison(
    ...
    for df, method in zip(dfs, methods):
        ...
        plt.plot(df_copy[fairness_metric], df_copy[ml_metric], label=method, color=METHOD_COLORS[method], marker=METHOD_SHAPES[method])
    # Add Vanilla, i.e. Fauci when lambda is 0
    vanilla_df = dfs[0][dfs[0]["lambda"] == 0]
    plt.plot(vanilla_df[fairness_metric], vanilla_df[ml_metric], label="Vanilla", color=METHOD_COLORS["vanilla"], marker=METHOD_SHAPES["vanilla"])
